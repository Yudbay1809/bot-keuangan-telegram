...
interface UserConversation {
  step: 'type' | 'amount' | 'category' | 'description' | 'custom_category';
  data: TempTransactionData;
}

const userConversations = new Map<number, UserConversation>();

function getConversation(ctx: MyContext): UserConversation | undefined {
  const key = ctx.from?.id;
  if (!key) return undefined;
  return userConversations.get(key);
}

function setConversation(ctx: MyContext, conv: UserConversation) {
  if (ctx.from?.id) {
    userConversations.set(ctx.from.id, conv);
  }
}

function clearConversation(ctx: MyContext) {
  if (ctx.from?.id) {
...
const mainKeyboard = Markup.keyboard([
  ...
  ['📈 Stats', '📋 Transaksi'],
  ['❓ Help']
] as any).oneTime().resize() as any;

// Keyboard menu for time range
const timeKeyboard = Markup.inlineKeyboard([
  [Markup.button.callback('Bulan Ini', 'report_this_month')],
  [Markup.button.callback('Bulan Lalu', 'report_last_month')],
  [Markup.button.callback('Tahun Ini', 'report_this_year')],
  [Markup.button.callback('Semua', 'report_all')],
]);
...
export function registerCommands(bot: Telegraf<MyContext>): void {
  ...
  bot.start(async (ctx) => {
    ...
    getOrCreateUser(userId, name, username);
  ...
  bot.command('report', async (ctx) => {
    try {
      const userId = ctx.from.id;
      const name = ctx.from.first_name + (ctx.from.last_name ? ' ' + ctx.from.last_name : '');
      getOrCreateUser(userId, name, ctx.from.username);
      ...
      const stats = getMonthlyStats(userId, year, month);
  ...
  bot.command('stats', async (ctx) => {
    try {
      ...
      const categoryStats = getCategoryStats(userId, startDate, endDate);
  ...
  bot.command('add', async (ctx) => {
    ...
    setConversation(ctx, {
      step: 'type',
      data: {} as TempTransactionData
    });
    
    await ctx.reply('Pilih jenis transaksi:', 
      Markup.inlineKeyboard([
        [Markup.button.callback('💰 Pemasukan', 'tx_type_income')],
        [Markup.button.callback('💸 Pengeluaran', 'tx_type_expense')],
      ])
    );
  });
  ...
  bot.on('message', async (ctx) => {
    ...
    if (conv.step === 'amount') {
      ...
      const allCategories = getCategories(ctx.from.id);
      
      const buttons = allCategories.map(cat => [Markup.button.callback(`${cat.emoji} ${cat.name}`, `tx_cat_${cat.name}`)]);
      buttons.push([Markup.button.callback('➕ Kategori Lain', 'tx_cat_custom')]);
      
      await ctx.reply('Pilih kategori:', Markup.inlineKeyboard(buttons));
      return;
    }
...
await ctx.answerCbQuery();
  ...
  bot.hears('➕ Tambah', async (ctx) => {
    ...
    // Trigger /add flow
    const userId = ctx.from.id;
    getOrCreateUser(userId, ctx.from.first_name + (ctx.from.last_name ? ' ' + ctx.from.last_name : ''), ctx.from.username);
    setConversation(ctx, { step: 'type', data: {} as TempTransactionData });
    await ctx.reply('Pilih jenis transaksi:', 
      Markup.inlineKeyboard([
        [Markup.button.callback('💰 Pemasukan', 'tx_type_income')],
        [Markup.button.callback('💸 Pengeluaran', 'tx_type_expense')],
  ...
  bot.hears('📊 Laporan', async (ctx) => {
    ...
    try {
      ...
      getOrCreateUser(userId, ctx.from.first_name + (ctx.from.last_name ? ' ' + ctx.from.last_name : ''), ctx.from.username);
      const now = new Date();
      const stats = getMonthlyStats(userId, now.getFullYear(), now.getMonth() + 1);
      const balance = stats.income - stats.expense;
  ...
  bot.command('export', async (ctx) => {
    try {
      ...
      const filePath = path.join(tempDir, `export_${userId}_${Date.now()}.csv`);
  ...
  bot.action('report_this_year', async (ctx) => {
    ...
    for (let m = 0; m < 12; m++) {
      const stats = getMonthlyStats(userId, now.getFullYear(), m + 1);
